  ...
  echo "$AVALIABLE_SERVICES" | while read SERVICE_ID BAR NAME
    ...
    then
     MAIN_MENU "That is not a number"
      ...
      RENDERED_SERVICES=$($PSQL "SELECT service_id FROM services WHERE service_id = $SERVICE_ID_SELECTED ")
      SERVICE_NAME=$($PSQL "SELECT name FROM services WHERE service_id = $SERVICE_ID_SELECTED ")
      if [[ -z $RENDERED_SERVICES ]]
        then
        ...
        if [[ $SERVICE_TIME ]]
        then
          SERVICES_RESULT=$($PSQL "INSERT INTO appointments(customer_id, service_id, time) VALUES($CUSTOMER_ID, $SERVICE_ID_SELECTED,'$SERVICE_TIME')")
          if [[ $SERVICES_RESULT ]]
          then
